...
public class UserController extends AbsUserController {
    ...
    @Method(name = "保存用户", desc = "保存用户", exceps = {@Exception(code = "001001010", message = "用户登录ID重复"),
        @Exception(code = "001001008", message = "用户数据验证失败")})
    public ModelAndView saveUser(@Param
    User user, HttpServletRequest request) {
        ...
        try {
            ...
            return processFailure(ERROR_500_CODE, moduleName + ADD_PAGE, user, "insert user failure!", e.getMessage());
        }
    ...
    public ModelAndView preEdit(@RequestParam("loginID")
    String loginID)
        throws ServiceException {
    ...
    User user, HttpServletRequest request)
        ...
        try {
            ...
            return processFailure(ERROR_500_CODE, moduleName + EDIT_PAGE, user, "update user failure!", e.getMessage());
        }
        return processSuccess(SUCCESS_PAGE, user);
    }
    
    @RequestMapping(value = "view")
    public ModelAndView view(@RequestParam("loginID")
    String loginID)
        throws ServiceException {
        User user = userService.findUser(loginID);
    ...
    public ModelAndView preConfig(@RequestParam("loginID")
    String loginID)
        throws ServiceException {
        ...
        return processSuccess("module/user/config", result, loginID);
    }
    
    @RequestMapping(value = "config")
    public ModelAndView config(@RequestParam("loginID")
    String loginID, @RequestParam("userID")
    Long userID, @RequestParam("groups")
    List<Long> groupIDs, @RequestParam("roles")
    List<String> roleCodes)
        ...
        return processSuccess("module/user/config", null, params);
    }
