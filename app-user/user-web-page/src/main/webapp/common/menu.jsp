...
<c:set var="menus" value="${_SITE_MAIN_DATA_.menus}"></c:set>
<!--  菜单区域 -->
<div class="col-md-2">
		...
		<ul class="nav bs-sidenav">
			...
			<c:forEach var="item" items="${menus}">
				<c:if test="${item.parentResID == -1 }">
					<!-- li class="active"-->
					<li><a href="${item.url}?resID=${item.resID}">${item.name }</a>
						<!-- 子菜单 -->
						<ul class="nav">
							<c:forEach var="childItem" items="${menus}">
								<c:if test="${childItem.parentResID == item.resID }">
									<li><a href="${childItem.url}?resID=${childItem.resID}">${childItem.name }</a></li>
								</c:if>
							</c:forEach>
						</ul></li>
				</c:if>
