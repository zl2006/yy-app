 ...
 * ========================================================= */
define("gallery/bootstrapvalidator/1.0.3/bootstrap3validator-debug", [ "$-debug" ], function(require, exports, module) {
    var $ = require("$-debug");
    module.exports = $;
    !function($) {
        ...
        $.fn.valid = function(options) {
            ...
            $("input, textarea", this).each(function() {
                var el = $(this), controlGroup = el.parents(".form-group"), //check-type="required chinese"  //支持多个，以空格隔开。
        ...
        var formState = false, fieldState = false, wFocus = false, fform_style = 0, //0=表示基本表单 1=表示内联表单 2=水平排列的表单
        globalOptions = {};
        ...
        var validateField = function(field, valid) {
            ...
            if (minlength && !error) {
                ...
                if (error && (msg == null || errorMsg == "")) {
                    errorMsg = "输入长度必须大于等于" + minlength;
                }
            }
            if (minlength && !error) {
                error = el.val().length > maxlength;
                if (error && (msg == null || errorMsg == "")) {
                    errorMsg = "输入长度必须小于等于" + maxlength;
                }
            }
            //值区间
            ...
            if (form) {
                ...
                if (fstyle == 0) {
                    ...
                    el.after('<span class="validation-error control-label" id="valierr">' + errorMsg + "</span>");
                } else if (fstyle == 1) {} else if (fstyle == 2) {
                    controlGroup.find("#valierr").remove();
                    //el.parent().after('<span class="validation-error control-label" id="valierr">' + errorMsg +'</span>');
                    el.parent().parent().append('<span class="validation-error control-label" id="valierr">' + errorMsg + "</span>");
                }
            }
            //end !form
            return !error;
        ...
        var validationForm = function(obj) {
            ...
            if (globalOptions.reqmark == true) {
                ...
                } else if (fform_style == 1) {} else if (fform_style == 2) {
                    $(obj).find("input, textarea").each(function() {
                        var el = $(this);
                        var controlGroup = el.parents(".form-group");
                        controlGroup.removeClass("has-error has-success");
                        ...
                        if (valid) {
                            if ($.inArray("required", valid) >= 0) {
                                el.parent().parent().append('<span class="validation-error" id="valierr" style="color:#FF9966">*</span>');
                            }
                        }
                    });
                }
