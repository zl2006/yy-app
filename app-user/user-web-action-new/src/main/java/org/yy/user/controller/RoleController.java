...
public class RoleController extends AbsUserController {
	...
	public ModelAndView list(@Param RoleDTO roleDTO) throws ServiceException {
		return processSuccess(moduleName + LIST_PAGE,
				roleService.findRole(roleDTO), roleDTO);
	}
	...
	public ModelAndView save(@Param Role role, HttpServletRequest request)
		...
		try {
			...
			return processFailure(ERROR_500_CODE, moduleName + ADD_PAGE, role,
					"insert role failure!", e.getMessage());
		}
		return processSuccess(SUCCESS_PAGE, role);
	}

	@RequestMapping(value = "update")
	public ModelAndView update(@Param Role role, HttpServletRequest request)
			throws ServiceException {
		Long userID = getCurrentUserID(request);
		role.setUpdatePerson(userID);
		try {
			...
			return processFailure(ERROR_500_CODE, moduleName + EDIT_PAGE, role,
					"update role failure!", e.getMessage());
		}
		return processSuccess(SUCCESS_PAGE, role);
	}
	...
	@RequestMapping(value = "enable", method = { RequestMethod.GET,
			RequestMethod.POST })
	public ModelAndView enable(@RequestParam("roleCode") String roleCode,
		...
		try {
			...
			return processFailure(ERROR_500_CODE, moduleName + LIST_PAGE, role,
					"update role status failure!", e.getMessage());
		}
		return processSuccess(SUCCESS_PAGE, role);
	}
	...
	@RequestMapping(value = "disable", method = { RequestMethod.GET,
			RequestMethod.POST })
	public ModelAndView disable(@RequestParam("roleCode") String roleCode,
			HttpServletRequest request) throws ServiceException {
		Role role = roleService.findRole(roleCode);
		Long userID = getCurrentUserID(request);
		role.setUpdatePerson(userID);
	...
	@RequestMapping(value = "preedit", method = { RequestMethod.GET,
			RequestMethod.POST })
	public ModelAndView preEdit(@RequestParam("roleCode") String roleCode)
			throws ServiceException {
		Role role = roleService.findRole(roleCode);
	...
	public ModelAndView view(@RequestParam("roleCode") String roleCode)
			throws ServiceException {
	...
	public ModelAndView preConfig(@RequestParam("roleCode") String roleCode)
			throws ServiceException {
		...
		return processSuccess(moduleName + "config", result, roleCode);
	}

	@RequestMapping(value = "config")
	public ModelAndView config(@RequestParam("roleCode") String roleCode,
			@RequestParam("resID") List<Long> resIDs) throws ServiceException {
		...
		return processSuccess(moduleName + "config", "", params);
	}
