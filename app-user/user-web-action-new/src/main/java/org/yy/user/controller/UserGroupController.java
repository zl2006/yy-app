...
public class UserGroupController extends AbsUserController {
	...
	public ModelAndView list(@Param UserGroupDTO userGroupDTO)
			throws ServiceException {
		return processSuccess(moduleName + LIST_PAGE,
				userGroupService.findUserGroup(userGroupDTO), userGroupDTO);
	}
	...
	@RequestMapping(value = "save", method = { RequestMethod.GET,
			RequestMethod.POST })
	public ModelAndView saveGroup(@Param UserGroup group,
		...
		try {
			...
			return processFailure(ERROR_500_CODE, moduleName + ADD_PAGE, group,
					"insert group failure!", e.getMessage());
		}
		return processSuccess(SUCCESS_PAGE, group);
	}

	@RequestMapping(value = "view")
	public ModelAndView view(@RequestParam("userGroupID") Long userGroupID)
			throws ServiceException {
	...
	public ModelAndView disable(@RequestParam("userGroupID") Long userGroupID)
			throws ServiceException {
		UserGroup userGroup = userGroupService.findUserGroup(userGroupID);
		if (userGroup != null && userGroup.getStatus() != 0) {
			...
			try {
				...
				return processFailure(ERROR_500_CODE, moduleName + LIST_PAGE,
						userGroup, "update usergroup status failure!",
						ex.getMessage());
			}
		}
	...
	public ModelAndView enable(@RequestParam("userGroupID") Long userGroupID,
		...
		if (userGroup != null && userGroup.getStatus() != 1) {
			...
			try {
				...
				return processFailure(ERROR_500_CODE, moduleName + LIST_PAGE,
						userGroupID, "update usergroup status failure!",
						ex.getMessage());
			}
		}
		return processSuccess(moduleName + LIST_PAGE, userGroup, userGroupID);
	...
	public ModelAndView preEdit(@RequestParam("userGroupID") Long userGroupID,
			HttpServletRequest request) throws ServiceException {
		return processSuccess(moduleName + "edit",
				userGroupService.findUserGroup(userGroupID), userGroupID);
	}
	...
	public ModelAndView edit(UserGroup userGroup, HttpServletRequest request)
		...
		try {
			...
			return processFailure(ERROR_500_CODE, moduleName + "edit",
					userGroup, "update usergroup failure!", e.getMessage());
		}
		return processSuccess(moduleName + LIST_PAGE, userGroup);
	}

	@RequestMapping(value = "preconfig")
	public ModelAndView preConfig(@RequestParam("userGroupID") Long userGroupID)
			throws ServiceException {
		...
		return processSuccess(moduleName + "/config", result, userGroupID);
	}

	@RequestMapping(value = "config")
	public ModelAndView config(@RequestParam("userGroupID") Long userGroupID,
