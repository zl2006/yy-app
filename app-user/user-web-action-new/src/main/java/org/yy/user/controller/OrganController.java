...
public class OrganController extends AbsUserController {
	...
	public ModelAndView list(@Param OrganDTO organDTO) throws ServiceException {

		// 当有参数条件时，使用参数条件，否则查顶层列表
		if (StringUtils.isEmpty(organDTO.getName())
				&& StringUtils.isEmpty(organDTO.getOrganCode())
				&& StringUtils.isEmpty(organDTO.getParentOrganCode())) {
			organDTO.setParentOrganCode("-1");
		}
		return processSuccess(moduleName + LIST_PAGE,
				organService.findOrgan(organDTO), organDTO);
	}
	...
	public ModelAndView listChild(
			@Param({ "parentOrganCode" }) OrganDTO organDTO)
	...
	@RequestMapping(value = "save", method = { RequestMethod.GET,
			RequestMethod.POST })
	@Method(name = "保存组织机构", desc = "保存组织机构", exceps = {
			@Exception(code = "001001010", message = "用户登录ID重复"),
			@Exception(code = "001001008", message = "用户数据验证失败") })
	public ModelAndView saveOrgan(@Param Organ organ, HttpServletRequest request) {
		...
		try {
			...
			return processFailure(ERROR_500_CODE, moduleName + ADD_PAGE, organ,
					"insert organ failure!", e.getMessage());
		}
	...
	@RequestMapping(value = "preedit", method = { RequestMethod.GET,
			RequestMethod.POST })
	public ModelAndView preEdit(@RequestParam("organCode") String organCode)
			throws ServiceException {
		Organ organ = organService.findOrgan(organCode);
		Organ parentOrgan = null;
		if (organ != null && organ.getParentOrganCode() != null
				&& !"-1".equals(organ.getParentOrganCode())) {
	...
	@RequestMapping(value = "view", method = { RequestMethod.GET,
			RequestMethod.POST })
	public ModelAndView view(@RequestParam("organCode") String organCode)
			throws ServiceException {
		Organ organ = organService.findOrgan(organCode);
		Organ parentOrgan = null;
		if (organ != null && organ.getParentOrganCode() != null
	...
	@RequestMapping(value = "edit", method = { RequestMethod.GET,
			RequestMethod.POST })
	public ModelAndView edit(@Param Organ organ, HttpServletRequest request)
			throws ServiceException {
		...
		try {
			...
			return processFailure(ERROR_500_CODE, moduleName + EDIT_PAGE,
					organ, "update organ failure!", e.getMessage());
		}
		return processSuccess(moduleName + LIST_PAGE, organ);
	}
	...
	@RequestMapping(value = "enable", method = { RequestMethod.GET,
			RequestMethod.POST })
	public ModelAndView enable(@RequestParam("organCode") String organCode,
		...
		try {
			...
			return processFailure(ERROR_500_CODE, moduleName + LIST_PAGE,
					organ, "update organ status failure!", e.getMessage());
		}
		return processSuccess(moduleName + LIST_PAGE, organ);
	}
	...
	@RequestMapping(value = "disable", method = { RequestMethod.GET,
			RequestMethod.POST })
	public ModelAndView disable(@RequestParam("organCode") String organCode,
			HttpServletRequest request) throws ServiceException {
		Organ organ = organService.findOrgan(organCode);
		Long userID = getCurrentUserID(request);
		organ.setUpdatePerson(userID);
