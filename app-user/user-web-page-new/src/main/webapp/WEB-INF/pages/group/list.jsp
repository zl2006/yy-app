        ...
        <div class="content">
            ...
            <table class="pure-table search-res" width="100%" id="data_table"><!--搜索结果 -->
                ...
                <c:forEach var="item" varStatus="status" items="${data.result}">
					<tr view="/group/view.do?userGroupID=${item.userGroupID}">
                    	...
                    	<c:forEach var="itemoper" items="${_SITE_MAIN_DATA_.listOperations }">
							<a href="${itemoper.url }?userGroupID=${item.userGroupID}" style="cursor: hand;" class="list_oper"  ename="${itemoper.ename }">${itemoper.name }</a>
						</c:forEach>
                    </td>
					</tr>
				</c:forEach>
