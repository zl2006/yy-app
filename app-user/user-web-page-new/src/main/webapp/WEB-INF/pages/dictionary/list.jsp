    ...
    <div id="content_wrap"><!--内容区-->
        <div class="content">
            ...
            <table class="pure-table search-res" width="100%" id="data_table"><!--搜索结果 -->
                ...
                <tr style="cursor: hand"  view="/dic/view.do?dicID=${item.dicID}" <c:if test="${ (status.index+1) % 2 == 0}">class="odd"</c:if>  >
                    ...
                    <td width="120">
                    	<c:forEach var="itemoper" items="${_SITE_MAIN_DATA_.listOperations }">
							<a href="${itemoper.url }?dicID=${item.dicID}" style="cursor: hand;" class="list_oper"  ename="${itemoper.ename }">${itemoper.name }</a>
						</c:forEach>
                    </td>
                </tr>
                </c:forEach>
