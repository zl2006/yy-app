    ...
    <div id="content_wrap"><!--内容区-->
        <div class="content">
            ...
            <form class="pure-form search" id="query_form" action="${basePath}/organ/list.do" method="post"><!--搜索表单-->
                ...
                <label for="organCode">机构编号：</label>
                ...
                <input name="status" type="text"  class="pure-u-1-5"  value="${params.status }">
            ...
            <table class="pure-table search-res" width="100%" id="data_table"><!--搜索结果 -->
                ...
                <tr style="cursor: hand"  view="/organ/view.do?organCode=${item.organCode}" <c:if test="${ (status.index+1) % 2 == 0}">class="odd"</c:if>    id="${item.organCode}" <c:if test="${item.hasChild == 1}"> haschild="true" </c:if> <c:if test="${item.parentOrganCode ne '-1'}"> pid="${item.parentOrganCode}"</c:if>  >
                    ...
                    <td width="120">
                    	<c:forEach var="itemoper" items="${_SITE_MAIN_DATA_.listOperations }">
							<a href="${itemoper.url }?organCode=${item.organCode}" style="cursor: hand;"  class="list_oper" ename="${itemoper.ename }">${itemoper.name }</a>
						</c:forEach>
                    </td>
                </tr>
                </c:forEach>
...
<script type="text/javascript">
    requirejs( [ 'jqsuperslide', /*'sticky',*/'organ_business' ], function (slide/*,sticky*/,Business) {
